...
#
# GTApprox Tutorial. Auxilary functions 
from da.macros import gtdoe
import numpy as np
...
########## Input generation
def input_generator(sample_size, doe='random', minmax_values=None, x_dim=2):
  
  if doe == 'random':
    x = np.random.rand(sample_size, x_dim) 
  else: # latin hypercube or full-factorial
    if doe == 'latin_hypercube':
      doe_options = {'GTDoE/Technique': 'LHS'}
    else:
      doe_options = {'GTDoE/Technique': 'FullFactorial'}
    generator = gtdoe.Generator()
    result = generator.generate(count=sample_size, options = doe_options,
                              bounds=(np.tile(0, (1, x_dim))[0], np.tile(1, (1, x_dim))[0]))
...
def plot_surface(calc_method, scatter_x=None, scatter_y=None, minmax_values = [[-1, 1], [-1, 1]], \
  ...
  if not scatter_x==None:# & not scatter_y=None:
